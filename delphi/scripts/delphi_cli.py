...
def submit_job(dynamodb, zid, job_type='FULL_PIPELINE', priority=50, 
    ...
    if job_type == 'FULL_PIPELINE':
        ...
        stages.append({
            "stage": "REPORT",
            "config": {
                "model": model if model else os.environ.get("GENAI_MODEL"), # Use provided model or env var
                "include_topics": True
            }
        })
        ...
        current_model = model if model else os.environ.get("GENAI_MODEL") # Must be set via arg or env var
        if not current_model:
            raise ValueError("Model must be specified via --model or GENAI_MODEL environment variable")
        ...
        if choice == "1":
            ...
            if job_type == "FULL_PIPELINE":
                if Confirm.ask("Set parameters for FULL_PIPELINE (max_votes, batch_size, model)?"):
                    ...
                    model_input = Prompt.ask("Model for REPORT stage (optional, defaults to GENAI_MODEL env var)", default="")
                    if model_input: model_param = model_input
            
            elif job_type == "CREATE_NARRATIVE_BATCH":
                report_id_stage_param = Prompt.ask("[bold]Report ID (for stage config)[/bold]")
                default_model = os.environ.get("GENAI_MODEL", "")
                if default_model:
                    model_param = Prompt.ask(f"[bold]Model[/bold] (defaults to {default_model})", default=default_model)
                else:
                    model_param = Prompt.ask("[bold]Model[/bold] (REQUIRED - set GENAI_MODEL env var to avoid this prompt)")
...
def main():
    ...
    submit_parser.add_argument("--model", help="Model to use (defaults to GENAI_MODEL env var)")
