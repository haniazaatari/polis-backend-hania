...
import { createConversation, populateConversationWithVotes, registerAndLoginUser } from '../setup/api-test-helpers.js';
...
describe('Data Export API', () => {
  ...
  beforeAll(async () => {
    ...
    // Get the report ID
    const getReportsResponse = await agent.get(`/api/v3/reports?conversation_id=${conversationId}`);
  ...
  test('GET /api/v3/reportExport/:report_id/summary.csv - should export report summary', async () => {
    const response = await agent.get(`/api/v3/reportExport/${reportId}/summary.csv`);
  ...
  test('GET /api/v3/reportExport/:report_id/comments.csv - should export comments', async () => {
    const response = await agent.get(`/api/v3/reportExport/${reportId}/comments.csv`);
  ...
  test('GET /api/v3/reportExport/:report_id/votes.csv - should export votes', async () => {
    const response = await textAgent.get(`/api/v3/reportExport/${reportId}/votes.csv`);
  ...
  test('GET /api/v3/reportExport/:report_id/unknown.csv - should handle unknown report type', async () => {
    const response = await textAgent.get(`/api/v3/reportExport/${reportId}/unknown.csv`);
  ...
  test('GET /api/v3/reportExport/nonexistent/comments.csv - should handle nonexistent report ID', async () => {
    const response = await textAgent.get('/api/v3/reportExport/nonexistent/comments.csv');
