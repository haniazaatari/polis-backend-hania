...
describe('Reports API', () => {
  ...
  test('POST /api/v3/reports - should create a new report', async () => {
    const response = await textAgent.post('/api/v3/reports').send({
    ...
    const getResponse = await textAgent.get(`/api/v3/reports?conversation_id=${conversationId}`);
    const reports = JSON.parse(getResponse.text);
  ...
  test('GET /api/v3/reports - should return reports for the conversation', async () => {
    ...
    const response = await textAgent.get(`/api/v3/reports?conversation_id=${conversationId}`);
  ...
  describe('with existing report', () => {
    ...
    test('PUT /api/v3/reports - should update report details', async () => {
      ...
      const response = await textAgent.put('/api/v3/reports').send({
        conversation_id: conversationId,
        report_id: reportId,
    ...
    test('GET /api/v3/reports - should get all reports for user', async () => {
      const response = await textAgent.get('/api/v3/reports');
    ...
    test('GET /api/v3/reports?report_id - should get a specific report', async () => {
      const response = await textAgent.get(`/api/v3/reports?report_id=${reportId}`);
