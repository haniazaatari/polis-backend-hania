...
import { createConversation, generateTestUser, registerAndLoginUser } from '../setup/api-test-helpers.js';
...
describe('Email Invites API', () => {
  ...
  test('POST /einvites - should create email invite and send welcome email', async () => {
    ...
    const response = await agent.post('/api/v3/einvites').send({
      email: testEmail
    });
  ...
  test('POST /users/invite - should handle invitation emails with error validation', async () => {
    ...
    for (const email of testEmails) {
      const sentEmail = await findEmailByRecipient(email);
  ...
  test('GET /verify - should handle email verification with error validation', async () => {
    ...
    const missingTokenResponse = await agent.get('/api/v3/verify');
  ...
  test('POST /sendCreatedLinkToEmail - should request email conversation link', async () => {
    ...
    const response = await agent.post('/api/v3/sendCreatedLinkToEmail').send({
      conversation_id: conversationId
    });
    ...
    const email = await findEmailByRecipient(testUser.email);
