...
describe('Authentication with Supertest', () => {
  // Use getters instead of direct access to global agents
  ...
  describe('Login Endpoint', () => {
    test('should validate login parameters', async () => {
      ...
      const noPasswordResponse = await textAgent.post('/api/v3/auth/login').send({});
      expect(noPasswordResponse.status).toBe(400);
      expect(noPasswordResponse.text).toContain('polis_err_param_missing_password');

      // Test missing email
      const noEmailResponse = await textAgent.post('/api/v3/auth/login').send({ password: 'testpass' });
      ...
      const invalidResponse = await textAgent.post('/api/v3/auth/login').send({
  ...
  describe('Registration Endpoint', () => {
    ...
    test('should validate registration parameters', async () => {
      ...
      const mismatchResponse = await textAgent.post('/api/v3/auth/new').send({
        ...validRegistration,
      ...
      await wait(500); // Add delay to prevent socket hang up

      // Test missing required fields
      const missingFieldsResponse = await textAgent.post('/api/v3/auth/new').send({
      ...
      const noTosResponse = await textAgent.post('/api/v3/auth/new').send({
        ...validRegistration,
  ...
  describe('Deregister (Logout) Endpoint', () => {
    test('should handle logout parameters', async () => {
      ...
      const noShowPageResponse = await textAgent.post('/api/v3/auth/deregister').send({});
      expect(noShowPageResponse.status).toBe(200);

      // Test null showPage
      const nullShowPageResponse = await textAgent.post('/api/v3/auth/deregister').send({
        showPage: null
      });
  ...
  describe('Register-Login Flow', () => {
    test('should complete full registration and login flow', async () => {
      ...
      const registerResponse = await agent.post('/api/v3/auth/new').send({
      ...
      const registerBody = JSON.parse(registerResponse.text);
  ...
  describe('Complete Auth Flow', () => {
    test('should handle complete auth lifecycle', async () => {
      ...
      // STEP 3: Logout user
      const logoutResponse = await textAgent.post('/api/v3/auth/deregister').send({});
      expect(logoutResponse.status).toBe(200);

      // STEP 4: Verify protected resource access fails
      const protectedResponse = await textAgent.get('/api/v3/conversations');
      expect(protectedResponse.status).toBe(403);
      expect(protectedResponse.text).toContain('polis_err_need_auth');

      // STEP 5: Verify can login again
      const reloginResponse = await agent.post('/api/v3/auth/login').send({
        email: completeFlowUser.email,
        password: completeFlowUser.password
      });
  ...
  describe('Participant Authentication', () => {
    ...
    test('should authenticate participant upon voting', async () => {
      ...
      const { agent, cookies, status } = await initializeParticipant(conversationId);
    ...
    test('should initialize participant with XID', async () => {
      ...
      const { agent, body, cookies, status } = await initializeParticipantWithXid(conversationId, xid);
