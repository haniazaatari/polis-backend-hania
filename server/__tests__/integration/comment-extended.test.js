...
describe('Extended Comment Endpoints', () => {
  ...
  test('GET /comments with tids - Get specific comment by ID', async () => {
    ...
    const newCommentId = await createComment(agent, conversationId, {
      txt: commentText
    });

    // Retrieve the specific comment by ID using the tids parameter
    const commentsResponse = await agent.get(`/api/v3/comments?conversation_id=${conversationId}&tids=${newCommentId}`);

    expect(commentsResponse.status).toBe(200);
    const comments = JSON.parse(commentsResponse.text);
  ...
  test('PUT /comments - Moderate a comment', async () => {
    ...
    const moderationCommentId = await createComment(agent, conversationId, {
      txt: commentText
    });

    // Moderate the comment - this endpoint is for moderation, not updating text
    const updateResponse = await agent.put('/api/v3/comments').send({
  ...
  test('PUT /comments - Validation fails for missing required fields', async () => {
    ...
    const response = await textAgent.put('/api/v3/comments').send({
  ...
  test('GET /comments - Filtering by multiple parameters', async () => {
    ...
    const comment1Id = await createComment(agent, conversationId, {
      txt: `Comment for filtering test 1 ${Date.now()}`
    });

    const comment2Id = await createComment(agent, conversationId, {
      txt: `Comment for filtering test 2 ${Date.now()}`
    });

    const comment3Id = await createComment(agent, conversationId, {
      txt: `Comment for filtering test 3 ${Date.now()}`
    });

    // Moderate comment 2
    const moderateResponse = await agent.put('/api/v3/comments').send({
      tid: comment2Id,
      conversation_id: conversationId,
      active: true,
      mod: 1,
    ...
    const filteredByModResponse = await agent.get(
    ...
    const filteredByMod = JSON.parse(filteredByModResponse.text);
