...
import { METRICS_IN_RAM, MPromise, addInRamMetric } from './utils/metered';
...
function initializePolisHelpers() {
  ...
  function doVotesPost(_uid, pid, conv, tid, voteType, weight, high_priority) {
    ...
    return new Promise((resolve, reject) => {
      ...
      pgQuery.queryP(query, params, (err, result) => {
        if (err) {
  ...
  function handle_POST_zinvites(req, res) {
    ...
    pgQuery.queryP('SELECT * FROM conversations WHERE zid = ($1) AND owner = ($2);', [req.p.zid, req.p.uid], (err) => {
      if (err) {
  ...
  function handle_PUT_reports(req, res) {
    ...
    return isModerator(zid, uid)
      .then((isMod, err) => {
        ...
        return pgQuery.queryP(query, []).then(() => {
          res.json({});
        });
      })
      .catch((err) => {
