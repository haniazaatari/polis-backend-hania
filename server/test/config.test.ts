...
describe("Config", () => {
  ...
  describe("getServerUrl", () => {
    ...
    test('returns API_DEV_HOSTNAME when DEV_MODE is true', async () => {
      ...
      jest.replaceProperty(process, 'env', {DEV_MODE: 'true', API_DEV_HOSTNAME: 'test.example.com:5050'});

      const { default: Config } = await import('../src/config');
      // No req object needed
      expect(Config.getServerUrl()).toBe('http://test.example.com:5050');
    });
    ...
    test('returns API_DEV_HOSTNAME when DEV_MODE is true', async () => {
      ...
      expect(Config.getServerUrl()).toBe('http://mydomain.xyz');
    });
  });
